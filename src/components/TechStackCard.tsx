    ...
    <div className="bg-white/95 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800/80 rounded-2xl p-6 shadow-xl dark:shadow-2xl shadow-slate-200/50 dark:shadow-black/50 space-y-6 backdrop-blur-xl">
      <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800/80">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="text-lg font-bold text-slate-900 dark:text-white tracking-tight">Languages & Tech Stack</h3>
        </div>
      ...
      <div>
        ...
        <div className="w-full h-3 bg-slate-100 dark:bg-slate-950 rounded-full overflow-hidden flex mb-3 border border-slate-200 dark:border-slate-800">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {sortedLanguages.map(([lang, count], idx) => {
            ...
            return (
              <div key={lang} className="flex items-center justify-between text-xs bg-slate-50 dark:bg-slate-950 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-800">
                <div className="flex items-center space-x-2">
                ...
                <span className="font-mono text-slate-500 dark:text-slate-400">{count} files ({pct}%)</span>
              </div>
            );
          })}
        </div>
        ...
        {facts.frameworks.length > 0 ? (
          <div className="flex flex-wrap gap-2">
            {facts.frameworks.map((fw) => (
              <span
                key={fw}
                className="inline-flex items-center space-x-1 px-3 py-1 rounded-lg bg-[#e8702a]/10 border border-[#e8702a]/30 text-[#e8702a] text-xs font-semibold whitespace-nowrap"
              >
      ...
      <div>
        ...
        <div className="max-h-36 overflow-y-auto space-y-1.5 pr-1">
          {facts.important_files.map((file) => (
            <div key={file} className="flex items-center space-x-2 text-xs font-mono bg-slate-50 dark:bg-slate-950 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300">
