  ...
  return (
    <div className="bg-slate-900/80 border border-slate-800/80 border-t-white/10 rounded-2xl p-6 shadow-2xl shadow-black/50 space-y-6 backdrop-blur-xl">
      <div className="flex items-center space-x-2 pb-3 border-b border-slate-800/80">
        ...
        <h3 className="text-lg font-bold text-white tracking-tight">Architecture & Learning Stratigraphy</h3>
      </div>
        ...
        {learningPath && learningPath.length > 0 ? (
          <div className="space-y-2.5">
            {learningPath.map((step, idx) => (
              <div
                key={idx}
                className="flex items-start space-x-3 p-3 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-300"
              >
                ...
                <div className="pt-0.5 leading-relaxed font-medium text-slate-200">
                  {step}
                </div>
              </div>
            ))}
          ...
          <p className="text-xs text-slate-500 italic">No learning steps generated yet.</p>
        )}
      </div>
    </div>
  );
