...
export const RagChatSection: React.FC<RagChatSectionProps> = ({ repoData, heightClass = "h-[820px]" }) => {
  ...
  return (
    <div className={`bg-slate-900/80 border border-slate-800/80 border-t-white/10 rounded-2xl p-6 shadow-2xl shadow-black/50 flex flex-col backdrop-blur-xl ${heightClass}`}>
      <div className="flex flex-wrap items-center justify-between pb-3 border-b border-slate-800/80 gap-2">
        <div className="flex items-center space-x-2.5">
          <div className="p-1.5 bg-slate-950 border border-purple-500/40 shadow-[0_0_12px_rgba(147,51,234,0.4)] rounded-xl flex items-center justify-center">
            <picture>
          ...
          <div>
            <h3 className="text-lg font-bold text-white tracking-tight">Interactive CodeSage Chat</h3>
            <p className="text-xs text-slate-400">Ask questions grounded in codebase context</p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className="hidden sm:flex px-2.5 py-1 text-xs font-mono font-semibold bg-[#e8702a]/10 text-[#e8702a] border border-[#e8702a]/30 rounded-full items-center space-x-1">
      ...
      <div className="flex-1 overflow-y-auto my-4 space-y-4 pr-1">
            ...
            <div
              className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 shadow-md ${
                msg.role === 'user'
                  ...
                  : 'bg-slate-950 text-[#e8702a] border border-slate-800'
              }`}
            >
              {msg.role === 'user' ? (
            ...
            <div
              className={`max-w-[85%] p-4 rounded-2xl text-xs sm:text-sm leading-relaxed shadow-lg ${
                msg.role === 'user'
                  ...
                  : 'bg-slate-950/90 border border-slate-800/80 text-slate-200 rounded-tl-none font-sans'
              }`}
            >
              <div className="markdown-body text-xs sm:text-sm font-sans space-y-1.5">
                <ReactMarkdown
                  components={{
                    ...
                    code: ({ children }) => (
                      <code className="bg-slate-800 text-[#e8702a] px-1.5 py-0.5 rounded text-[11px] font-mono border border-slate-700">
                        {children}
                      </code>
                    ),
                    pre: ({ children }) => (
                      <pre className="bg-slate-900 p-3 rounded-lg overflow-x-auto border border-slate-800 text-xs font-mono my-2 text-slate-100">
                        {children}
                      </pre>
                    ),
              ...
              <div
                className={`text-[10px] mt-2 font-mono ${
                  msg.role === 'user' ? 'text-white/80 text-right' : 'text-slate-500'
                }`}
              >
                {msg.timestamp}
              </div>
        ...
        {loading && (
          ...
          >
            <div className="w-8 h-8 rounded-xl bg-slate-950 text-[#e8702a] border border-slate-800 flex items-center justify-center">
        ...
        <div className="flex flex-wrap gap-1.5">
          {SAMPLE_QUESTIONS.map((q, idx) => (
            <motion.button
              ...
              disabled={loading}
              className="text-[11px] bg-slate-950/80 hover:bg-slate-800/80 text-slate-300 hover:text-white border border-slate-800 hover:border-slate-700 px-2.5 py-1 rounded-lg transition-colors cursor-pointer disabled:opacity-50"
            >
              {q}
            </motion.button>
          ))}
      ...
      >
        <input
          ...
          disabled={loading}
          className="flex-1 bg-slate-950/90 border border-slate-800 focus:border-[#e8702a] focus:ring-1 focus:ring-[#e8702a] text-white placeholder-slate-500 rounded-xl px-4 py-2.5 text-xs sm:text-sm font-sans transition-all disabled:opacity-50"
        />
        <motion.button
          whileTap={{ scale: 0.95 }}
          type="submit"
